...
// --- CONFIGURARE ---
// IMPORTANT: Nu ține token-ul direct în frontend. Mută-l pe backend / proxy și folosește variabile de mediu.
// În plus, token-ul pe care l-ai lipit în chat este considerat compromis: rotește-l imediat în Monday.com.
const API_KEY = "REPLACE_WITH_SERVER_SIDE_TOKEN";
...
const DEPARTMENTS = {
  ...
  operational: {
    name: "Operațiuni",
    employees: [
      { id: 101, name: "Alin Lita", mondayUserId: 73962695 },
      { id: 102, name: "Bogdan Serafim", mondayUserId: 73962698 },
...
const extractNumericValue = (columnValue) => {
  ...
  let valStr = "";
  if (columnValue.display_value) {
    valStr = String(columnValue.display_value);
  } else if (columnValue.text) {
    valStr = columnValue.text;
...
const getPersonIds = (columnValue) => {
  ...
  try {
    const parsed = JSON.parse(columnValue.value);
    return parsed.personsAndTeams ? parsed.personsAndTeams.map((p) => p.id) : [];
  } catch (e) {
    return [];
  }
};
...
const OperationalRowCells = ({ row, showSalesMetrics = false }) => {
  ...
  const rataConversieClienti = totalLeadsInteractions > 0 ? ((qualified / totalLeadsInteractions) * 100).toFixed(1) : "0.0";

  const solicitari = safeVal(row.solicitariCount);
  const websiteCount = safeVal(row.websiteCount);
  const conversionRateWebsite = solicitari > 0 ? ((websiteCount / solicitari) * 100).toFixed(1) : websiteCount > 0 ? "100.0" : "0.0";
  ...
  return (
    <React.Fragment>
      <td className="px-3 py-2 font-medium text-slate-900 sticky left-0 bg-white border-r shadow-sm min-w-[150px] z-10">
        {row.name}
      </td>
...
const TableHeader = ({ showSalesMetrics }) => (
  <thead className="text-[10px] uppercase bg-slate-100 border-b border-slate-200 text-slate-600">
    <tr>
      ...
      {showSalesMetrics && (
        <React.Fragment>
          <th className="px-2 py-3 text-center bg-yellow-100/50">
            Contactați<br />Tel
          </th>
          <th className="px-2 py-3 text-center bg-yellow-100/50">Calificați</th>
          <th className="px-2 py-3 text-center bg-yellow-100/50 border-r text-blue-700 font-bold">
            Rată conv.<br />Clienti
          </th>
      ...
      <th colSpan={5} className="px-2 py-2 text-center bg-blue-50 border-b border-blue-200 border-r text-blue-800 font-bold">
        După Data Contract
      </th>
    ...
    <tr>
      ...
      {showSalesMetrics && (
        <React.Fragment>
          <th className="bg-yellow-100/50" />
          <th className="bg-yellow-100/50" />
          <th className="bg-yellow-100/50 border-r" />
          <th className="bg-indigo-50" />
          <th className="bg-indigo-50 border-r" />
        </React.Fragment>
      )}
      ...
      <th className="px-1 py-2 text-center bg-blue-100/50 border-r font-bold">TOTAL</th>
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow border border-slate-200">
      <div className="p-4 border-b border-slate-200 bg-slate-50">
        <h3 className="font-bold text-slate-800">Departamentul de Operațiuni în intervalul: {dateRange}</h3>
      </div>
      <table className="w-full text-xs text-left text-slate-600 whitespace-nowrap">
        ...
        <tbody>
          ...
          {data.length === 0 && (
            <tr>
              <td colSpan={26} className="px-4 py-8 text-center text-slate-500">
...
const SalesTable = ({ data, dateRange }) => {
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow border border-slate-200">
      <div className="p-4 border-b border-slate-200 bg-slate-50">
        <h3 className="font-bold text-slate-800">Departamentul de Vânzări în intervalul: {dateRange}</h3>
      </div>
      <table className="w-full text-xs text-left text-slate-600 whitespace-nowrap">
        ...
        <tbody>
          ...
          {data.length === 0 && (
            <tr>
              <td colSpan={29} className="px-4 py-8 text-center text-slate-500">
                Nu există date disponibile pentru perioada selectată.
              </td>
            </tr>
          )}
...
export default function App() {
  ...
  const handleExport = () => {
    if (!opsStats.length && !salesStats.length) {
    ...
    const renderTableRows = (stats, isSales) => {
      return stats
        .map((row) => {
          ...
          const conversionRateWebsite =
            row.solicitariCount > 0
              ? ((row.websiteCount / row.solicitariCount) * 100).toFixed(1)
              : row.websiteCount > 0
              ? '100.0'
              : '0.0';
          ...
          let salesCells = '';
          if (isSales) {
            salesCells = `<td class="bg-yellow">${safeVal(row.contactat)}</td><td class="bg-yellow">${safeVal(row.calificat)}</td><td class="bg-yellow bold">${rataConversieClienti}%</td><td class="bg-indigo">${safeVal(
              row.emailsCount
            )}</td><td class="bg-indigo">${safeVal(row.callsCount)}</td>`;
          }
          ...
          return `<tr>
            ...
            <td>${formatCurrency(safeVal(row.profitRonRaw))}</td>
            <td class="bold bg-blue">${avgProfitability}%</td>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th colspan="5" class="bg-blue">După Data Contract</th>
              <th colspan="5" class="bg-green">După Data Livrare</th>
              <th rowspan="2">Profit RON<br/>(Brut)</th>
          ...
          <tbody>${renderTableRows(opsStats, false)}</tbody>
        </table>`;
    }
    ...
    if (salesStats.length) {
      content += `<h2>Departament Vânzări</h2>
        <table>
          ...
          <tbody>${renderTableRows(salesStats, true)}</tbody>
        </table>`;
    }
  ...
  const fetchMondayData = async ({ start, end }) => {
    ...
    try {
      ...
      // Build Sales User ID Set for filtering
      const salesUserIds = new Set(DEPARTMENTS.sales.employees.map((e) => String(e.mondayUserId)));

      // Helper to check if item is owned by sales team
      ...
      const furnizori = await fetchAllItems(
        BOARD_ID_FURNIZORI,
        Object.values(COLS.FURNIZORI),
        `[{ column_id: "${COLS.FURNIZORI.DATA}", operator: between, compare_value: ["${dateFrom}", "${dateTo}"] }]`
      );
      ...
      // --- OPTIMIZED ACTIVITIES FETCH VIA SERVER-SIDE FILTER (DATE ONLY) ---
      setStatusMessage('Se scanează itemii pentru activități (Filtrare Dată)...');

      // 1. Fetch Items filtered by Date from LEADS
      const rawLeads = await fetchAllItems(
        BOARD_ID_LEADS,
        ['name', COLS.LEADS.OWNER],
        `[{ column_id: "${COLS.LEADS.DATA}", operator: between, compare_value: ["${dateFrom}", "${dateTo}"] }]`
      );

      // 2. Fetch Items filtered by Date from CONTACTS
      const rawContacts = await fetchAllItems(
        BOARD_ID_CONTACTE,
        ['name', COLS.CONTACTE.OWNER],
        `[{ column_id: "${COLS.CONTACTE.DATA}", operator: between, compare_value: ["${dateFrom}", "${dateTo}"] }]`
      );

      // 3. Filter Client-Side by Owner
      const relevantLeads = rawLeads.items_page.items.filter((i) => isOwnedBySales(i, COLS.LEADS.OWNER));
      const relevantContacts = rawContacts.items_page.items.filter((i) => isOwnedBySales(i, COLS.CONTACTE.OWNER));

      const allActivityItemIds = [...relevantLeads, ...relevantContacts].map((i) => i.id);

      let activities = [];
      if (allActivityItemIds.length > 0) {
        setStatusMessage(`Se descarcă activități pentru ${allActivityItemIds.length} itemi relevanți...`);
        activities = await fetchActivitiesForItems(allActivityItemIds, start, end);
        console.log(`Found ${activities.length} activities`, activities.slice(0, 5));
      }

      setStatusMessage('Se finalizează calculele...');
      processAllData({ comenziCtr, comenziLivr, solicitari, leadsContact, leadsQualified, furnizori, activities, start, end });
    } catch (err) {
      console.error(err);
  ...
  const processAllData = ({ comenziCtr, comenziLivr, solicitari, leadsContact, leadsQualified, furnizori, activities, start, end }) => {
    setDateRangeStr(`${start.toLocaleDateString('ro-RO')} – ${end.toLocaleDateString('ro-RO')}`);

    const generateStats = (employees) =>
      employees.map((emp) => ({
        id: emp.id,
        name: emp.name,
    ...
    if (comenziCtr) {
      comenziCtr.items_page.items.forEach((item) => {
        ...
        const principalIds = getPersonIds(colPrincipal);
        const secondaryIds = getPersonIds(colSecundar);
        const hasSecondary = secondaryIds.length > 0;

        applyToAllStats((statsList) => {
          statsList.forEach((emp) => {
            const isPrincipal = principalIds.includes(emp.mondayId);
            const isSecondary = secondaryIds.includes(emp.mondayId);
    ...
    if (solicitari) {
      solicitari.items_page.items.forEach((item) => {
        ...
        applyToAllStats((statsList) => {
          statsList.forEach((emp) => {
            if (principalIds.includes(emp.mondayId)) {
              emp.solicitariCount++;
            }
          });
        });
    ...
    if (furnizori) {
      const personColId = COLS.FURNIZORI.PERSON;
      furnizori.items_page.items.forEach((item) => {
        const getCol = (id) => item.column_values.find((c) => c.id === id);
        const personVal = getCol(personColId);
        const personIds = getPersonIds(personVal);

        applyToAllStats((statsList) => {
          statsList.forEach((emp) => {
            if (personIds.includes(emp.mondayId)) {
              emp.suppliersAdded++;
            }
          });
        });
    ...
    if (activities && activities.length > 0) {
      activities.forEach((act) => {
        const userId = act.user?.id ? String(act.user.id) : null;
        if (!userId) return;
    ...
    const processLeads = (boardData, type) => {
      ...
      boardData.items_page.items.forEach((item) => {
        ...
        applyToAllStats((statsList) => {
          statsList.forEach((emp) => {
            if (ownerIds.includes(emp.mondayId)) {
      ...
      <div className="max-w-[95%] mx-auto">
        <div className="mb-8 border-b border-slate-200 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-2 text-sm">Generează tabele de activitate pentru Operațiuni și Vânzări.</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <button
              onClick={handleExport}
              disabled={!opsStats.length && !salesStats.length}
              className="flex items-center gap-2 text-white bg-blue-600 hover:bg-blue-700 text-sm font-medium border px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 min-w-[200px]">
              ...
              <select
                className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg outline-none cursor-pointer"
            ...
            <button
              ...
              disabled={loading}
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg transition-colors flex items-center justify-center gap-2"
            >
        ...
        {error && (
          <div className="p-4 bg-red-50 text-red-800 rounded-lg mb-6 flex gap-2">
        ...
        <div className="space-y-12">
          {opsStats.length > 0 && (
            <div className="animate-fade-in">
              <OperationalTable data={opsStats} dateRange={dateRangeStr} />
            </div>
          )}
          {salesStats.length > 0 && (
            <div className="animate-fade-in">
              <SalesTable data={salesStats} dateRange={dateRangeStr} />
            </div>
          )}
        </div>
      </div>
